...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    for r in range(1, H):
        prev_row = [np.roll(cost[r-1], -1), cost[r-1], np.roll(cost[r-1], 1)]
        cost[r] = energy[r] + np.amin( prev_row, axis = 0)
        cost[r, 0] = energy[r, 0] + min(cost[r-1, 0], cost[r-1, 1])
        cost[r, W-1] = energy[r, W-1] + min(cost[r-1, W-2], cost[r-1, W-1])

        paths[r] = 1 - np.argmin( prev_row, axis = 0)
        paths[r, 0] = 1 if cost[r-1, 0] > cost[r-1, 1] else 0
        paths[r, W-1] = -1 if cost[r-1, W-2] < cost[r-1, W-1] else 0
    ### END YOUR CODE
...
def remove_seam(image, seam):
    ...
    H, W, C = image.shape
    ### YOUR CODE HERE
    out = np.array(image[:, :W-1], copy = True)

    for r in range(H):
        out[r, seam[r]:] = image[r, seam[r]+1:]
    ### END YOUR CODE
...
def duplicate_seam(image, seam):
    ...
    ### YOUR CODE HERE
    out[:, :W] = np.array(image, copy = True)

    for r in range(H):
        out[r, seam[r]+1:] = image[r, seam[r]:]
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    ### YOUR CODE HERE
    k = size - W
    seams_ids = find_seams(out, k, efunc = efunc, cfunc = cfunc)
    seams = np.zeros((k, H), dtype = int)
    for i in range(k):
        seams[i] = np.where(seams_ids == i+1)[1]
        if i > 0:
            seams[i] += np.sum(np.int8(seams[i] > seams[:i]), axis = 0)

    # for si in range(k, 0, -1):
    #     seam = np.where(seams == si)[1]
    #     out = duplicate_seam(out, seam)
    for seam in seams:
        out = duplicate_seam(out, seam)
    ### END YOUR CODE
...
def compute_forward_cost(image,  ):
    ...
    for r in range(1, H):
        ...
        cost[r] = energy[r] + np.amin( prev_row, axis = 0)
        cost[r, 0] = energy[r, 0] + min(cost[r-1, 0], cost[r-1, 1])
        cost[r, W-1] = energy[r, W-1] + min(cost[r-1, W-1], cost[r-1, W-2])
        print (prev_row)
        paths[r] = 1 - np.argmin( prev_row, axis = 0)
        paths[r, 0] = 1 if cost[r-1, 0] > cost[r-1, 1] else 0
        paths[r, W-1] = -1 if cost[r-1, W-2] < cost[r-1, W-1] else 0
...
def energy_function_object_removal(image, mask):
    """Computes energy of the input image.
    ...
    for col in W:
        if np.sum(mask[:, col]) > 0:
            x = np.min(np.where(mask[:,col] == 1)[0])
            out[x:, col] = -1e5
    ### END YOUR CODE

    return out

def reduce_object_removal(image, mask, size, axis=1, efunc=energy_function_object_removal, cfunc=compute_forward_cost):
    """Reduces the size of the image using the seam carving process. Faster than `reduce`.
    ...
    out = np.copy(image)
    masked_img = np.copy(mask)
    if axis == 0:
        out = np.transpose(out, (1, 0, 2))
        mask = np.transpose(masked_img, (1, 0, 2))
    ...
    ### YOUR CODE HERE
    for _ in range(W - size):
        energy = efunc(out, masked_img)
        cost, paths = cfunc(out, energy)
        end = np.argmin(cost[-1])
        seam = backtrack_seam(paths, end)
        out = remove_seam(out, seam)
        masked_img = remove_seam(masked_img, seam)
    ### END YOUR CODE
...
def remove_object(image, mask):
    ...
    H, W, C = image.shape
    x, y = np.where(mask == 1)
    mask_height = np.max(x) - np.min(x) + 1
    mask_width = np.max(y) - np.min(y) + 1

    axis = 1
    size = min(mask_height, mask_width)
    size_reduce = W - size
    size_enlarge = W
    ...
    out = reduce_fast(image, size_reduce, axis)

    out = enlarge(out, size_enlarge, axis)
    ### END YOUR CODE
