...
def conv(image, kernel):
    ...
    for imgi in range(Hi):
        for imgj in range(Wi):
            out[imgi, imgj] = np.sum(padded[imgi:imgi+Hk, imgj:imgj+Wk] * flipped_kernel)
    ### END YOUR CODE
...
def gaussian_kernel(size, sigma):
    ...
    ### YOUR CODE HERE
    gaus_const = 1.0/(2*np.pi*(sigma ** 2))
    kernel = np.array([[(i-int(size/2))**2 + (j-int(size/2))**2 for j in range(size)] for i in range(size)])
    kernel = gaus_const * np.exp(-kernel/(2 * sigma ** 2))
    ### END YOUR CODE
...
def partial_x(img):
    ...
    ### YOUR CODE HERE
    x_filter = np.array([0.5, 0, -0.5]).reshape((1,3))
...
def partial_y(img):
    ...
    ### YOUR CODE HERE
    x_filter = np.array([0.5, 0, -0.5]).reshape((3,1))
    out = conv(img, x_filter)
    ### END YOUR CODE
...
def non_maximum_suppression(G, theta):
    ...
    for row in range(H):
        for col in range(W):
            largest = G[row, col]
            if theta[row, col] % 180 == 0 and col > 0 and col < W-1:
                largest = np.max(G[row, col-1:col+2])
...
def link_edges(strong_edges, weak_edges):
    ...
    for idx in indices:
        ...
        # nodes = [(node[0], node[1])]
        nodes = []
        edges[node[0], node[1]] = 1
        neighbors = get_neighbors(node[0], node[1], H, W)
        nodes += neighbors
...
def canny(img, kernel_size=5, sigma=1.4, high=20, low=15):
    ...
    """
    ### YOUR CODE HERE
    gkernel = gaussian_kernel(kernel_size, sigma)
    gfiltered_img = conv(img, gkernel)
    G, theta = gradient(gfiltered_img)
